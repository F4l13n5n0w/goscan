...
func main() {
	...
	if portsRange != "" {
		if portsRange == "-" {
			fmt.Println("scan full range of ports!")
			golbalfullports = true
			for i := 1; i <= 65535; i++ {
		...
		fmt.Println("no -p received, will use default top1k ports")
		scanports = top1kports[0:]
	}

	if singleIP == "" {
		//fmt.Println("not -ip received, should use -iL to specify a list of targets")
	} else {
		globalAddr = singleIP
		fmt.Printf("\n\n[+] scan %s\n", singleIP)
		portscan(scanports)
