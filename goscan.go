...
var globalAddr = "scanme.nmap.org"
...
func fullportscan() {
	log.Println("[+] Start full port scan...")
	ports := make(chan int, globalThread)
	results := make(chan int)
...
func main() {
	...
	var fullRange = flag.Int("fr", 0, "full ports range, should always be 65535")
	var nTimeout = flag.Int("st", 5, "TCP scan connection timeout (seconds)")
	var nTCPReaderTimeout = flag.Int("rt", 5, "TCP message reader timeout (seconds)")
