...
var globalTimeout int = 5
var globalTCPReaderTimeout int = 5
var globalThread int = 100
var globalAddr = "127.0.0.1"

var ServicesMapFilename = "services_tcp.csv"
var serviceTCPTable = []string{}
...
func portscan(scanports []int) {
	...
	for i := 0; i < len(scanports); i++ {
		...
		if port != 0 {
			...
			serviceTCPDefault := portServiceMap(port)
			fmt.Printf("%-10v%-10v%-20v%s\n", port, "open", serviceTCPDefault, extraInfo)
		}
	}
...
func loadDefaultServicesTable() {
	fileBytes, err := ioutil.ReadFile(ServicesMapFilename)
	if err != nil {
		fmt.Println(err)
		os.Exit(1)
	}
	serviceTCPTable = strings.Split(string(fileBytes), "\n")
}
...
func portServiceMap(port int) string {
	...
	for i := 0; i < len(serviceTCPTable); i++ {
		split := strings.Fields(serviceTCPTable[i])
		if key == split[1] {
			portService = split[0]
			break
		}
